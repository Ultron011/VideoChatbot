...
from livekit.plugins.liveavatar.api import LiveAvatarAPI, LiveAvatarException
from livekit.plugins.liveavatar.log import logger
...
class AvatarSession(BaseAvatarSession):
    ...
    def _on_clear_buffer(self) -> None:
        ...
        async def _handle_clear_buffer(audio_playing: bool) -> None:
            if audio_playing:
                ...
                # Always tell HeyGen to drop its remote buffer while audio is
                # in flight — gating on _avatar_speaking raced with chunked
                # sends (speak_started may not have arrived yet) and left the
                # avatar reading out a stale buffer after an interruption.
    ...
    async def _main_task(self) -> None:
        ...
        async def _forward_audio() -> None:
            ...
            chunk_duration = 0.0
            is_first_chunk = True
            first_chunk_threshold = 0.15  # 150ms (patched from 600ms)
            subsequent_chunk_threshold = 0.25  # 250ms (patched from 1s)

            # Pacing: TTS generates audio much faster than realtime, so an
            # unthrottled forward loop ships an entire long answer into
            # HeyGen's remote buffer within seconds. Interrupts then have to
            # kill a huge remote buffer (and often don't), so the avatar
            # keeps talking and new replies queue behind the old audio.
            # Keep at most MAX_LEAD_S of audio in flight; the rest stays in
            # the local queue where clear_buffer can actually drop it.
            MAX_LEAD_S = 2.5
            sent_duration = 0.0
            ...
            def _flush_chunk() -> None:
                ...
                nonlocal sent_duration, send_start
                if not chunk_buf:
                    return
            ...
            async for audio_frame in self._audio_buffer:
                ...
                if isinstance(audio_frame, rtc.AudioFrame):
                    ...
                    for resampled_frame in self._resample_audio(audio_frame):
                        if self._chunk_interrupted.is_set():
                            break  # outer loop discards on its next pass
                        ...
                        if chunk_duration >= threshold:
                            _flush_chunk()
                            # Throttle so HeyGen never holds more than
                            # MAX_LEAD_S of unplayed audio. Abort the wait
                            # immediately on interruption.
                            if send_start is not None:
                                ahead = sent_duration - (time.monotonic() - send_start)
                                if ahead > MAX_LEAD_S:
                                    try:
                                        await asyncio.wait_for(
                                            self._chunk_interrupted.wait(),
                                            timeout=ahead - MAX_LEAD_S,
                                        )
                    ...
                    is_first_chunk = True
                    # Each reply paces independently; an idle gap must not
                    # bank wall-clock credit for the next one.
                    sent_duration = 0.0
                    send_start = None
