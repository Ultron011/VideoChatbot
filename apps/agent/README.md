...
Run `python worker.py start`. The process must stay alive — deploy as a
long-running service (systemd unit, pm2, or any process manager). On the
current prod box it runs directly in its venv on the same host as nginx and
the token server.
